...
and check_ptypeexp sev pt0 = match pt0 with
  ...
  | TFun(i,xs,pt_sort,pt) -> 
      begin
	match xs with 
	    [] -> failAt i "Fatal error: zero-argument type function"
	  | [x] ->
	  ...
	  let map_impl = Safelist.fold_left
	    (fun f (i,n,l) ->
		 ...
		 fun v -> match v with
		   ...
		   | _ -> failAt i "Run error: argument to map is not a name")
	...
	begin
	  let e1_rv = compile_exp cev e1 in
	    match sort_of_rv e1_rv, value_of_rv e1_rv with
	      ...
	      | _ -> failAt i 
		  (sprintf "Run error: function expected at left-hand side of application but %s found"
...
and compile_exp_name cev e =
  ...
  let e_rv = compile_exp cev e in
    match value_of_rv (e_rv) with
      | Value.N n -> n
      | _ -> failAt i (sprintf "Run error: name expected but %s found" 
...
and compile_exp_view cev e =
  ...
  let e_rv = compile_exp cev e in
    match value_of_rv (e_rv) with
      | Value.V v -> v
      | _ -> failAt i (sprintf "Run error: view expected but %s found" 
			 (string_of_sort (sort_of_rv e_rv)))
...
and compile_bindings cev bs =
  ...
  let rec compile_binding cev bi =
	...
	let memoized_rv = make_rv (sort_of_rv e_rv) memoized_v in
      ...
      | Syntax.BDef(i,_,_,_,_) -> failAt i "Run error: unflattened binding"
  in
...
and compile_typebindings cev tbs =
  ...
  let rec compile_typebinding cev tbi =
	...
	| (x,_,_) -> failAt (info_of_id x) "Run error: unflattened type binding" 	    
  in
