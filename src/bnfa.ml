...
let min_char = Char.chr char_code_min
...
module NFA = struct
  ...
  module QS = Int.Set          (* sets of states *)
  module QM = Int.Map          (* maps keyed by states *)
...
*)
  ...
  let is_singleton t = 
    try 
      (is_empty (mk_diff t (mk_str (representative t))))
  ...
  let str_of_list l = 
    ...
    let a = String.make n (Char.chr 0) in
...
(*begin
  ...
  let unambig_star t = 
    if not (QS.is_empty (QS.inter t.init t.final)) then Misc.Left ("", -1, true) else
      (let tt = trim t in
		 ...
		 let t' = reset_heur {dt with final = QS.singleton q} in
		   try 
		     ...
		     let end_word = representative inter in
		       Misc.Left (beg_word^end_word, String.length beg_word, false)
		   with
...
let example_of_dss dss = 
  ...
  let n = String.length s in 
    ((String.sub s 0 i1, String.sub s i1 (n-i1)),
    (String.sub s 0 i2, String.sub s i2 (n-i2)))
...
type dual_multi_split = { dms_example : String.t;
			  dms_cut1 : int list;
			  dms_cut2 : int list}
...
let unambig_star_split t s = 
  Safelist.rev 
    (snd (Safelist.fold_left 
             (fun (j,acc) i -> (i,String.sub s j (i-j)::acc))
             (0,[])
