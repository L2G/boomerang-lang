...
let get_atom f0 = 
  let fvs = fvs_t f0 in 
  let zs = get_zeros f0 in
    if IntSet.cardinal zs = IntSet.cardinal fvs - 1 
    then 
      let x = IntSet.choose (IntSet.diff fvs zs) in
      let subst = IntSet.fold (fun zi s -> (zi,zero)::s) zs [] in
      let f0' = substitute ((x,Var(0))::subst) f0 in
        Some(f0')
    else None
