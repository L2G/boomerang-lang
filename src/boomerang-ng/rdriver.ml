...
let parse_lexbuf lexbuf = 
  try Rparser.modl Rlexer.main lexbuf 
...
let compile_boom_str s n = 
  let _ = Rlexer.setup "<string constant>" in    
...
let init () = 
  Rregistry.compile_file_impl := compile_file;
  Rregistry.compile_boom_str_impl := compile_boom_str;
