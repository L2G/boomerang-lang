...
let rec bookmark_to_desc : (bookmark -> V.desc) =
  function
	...
	let link = 
	  match c with 
	    NotUrl s ->
	      V.V ([("name",V.Val name);("noturl",V.Val s)])
	  | Url s -> 
	      let desc_of_url = function
		  NoDefault, InetShortcut l ->
		    V.V [("InetShortcut", V.V (List.map (fun (n,s) -> (n, V.Val s)) l))]
		| Default l1, InetShortcut l2 ->
		    V.V [("InetShortcut", V.V (List.map (fun (n,s) -> (n, V.Val s)) l2));
			  ("Default", V.V (List.map (fun (n,s) -> (n, V.Val s)) l1))]
		| _ -> assert false
	      in
  ...
  List.map 
	...
	let url =
	  match V.get link "url" with
	    Some urlt -> (".url", Url (url_from_tree urlt))
	  | None -> ("", NotUrl (V.get_value (V.get_required link "noturl")))	in
	File ((V.get_value name)^(fst url), (snd url))
    | _ -> assert false
	  )
    linklists
...
let unquote = 
  let uq (a,b) =
    let b' = if b.[0] = '"' then String.sub b 1 (String.length b - 2) else b in
      (a, b')
  in function
	(NoDefault, InetShortcut l) -> 
	  (NoDefault, InetShortcut (List.map uq l))
    | (Default l1, InetShortcut l2) ->
	(Default (List.map uq l1), InetShortcut (List.map uq l2))
    | _ -> assert false
	      
let rec skip_url_ext = function
    Folder (n, b, l) ->
     Folder(n, b, List.map skip_url_ext l)
  | File (name, url) ->
      (match url with
	   Url u -> File( String.sub name 0 (String.length name - 4), Url(unquote u))
	 | NotUrl _ -> File (name, url)
      )
...
let read_bookmark file =
  bookmark_to_tree (skip_url_ext (readBookmarkFolder file))
    
let dump_to_meta t f =
  let outc = open_out f in
    protect (fun () -> (output_string outc (Meta.writer t); close_out outc))
      
let read_from_meta f =
  protect (fun () -> Meta.reader (contents f))
