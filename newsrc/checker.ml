...
(** expressions **)
(* returns a pair: a sort and a new expression *)
let rec sc_exp sev e0 = match e0 with
  ...
  | EFun(i,(p1::p2::ps),so,e) ->
      ...
      let body_sort,body_exp = sc_exp fev (EFun(i,p2::ps,so,e)) in
  ...
  | EMap(i,ms) ->
	 ...
	 expression has the right sort *)
      let new_ms,new_sev = Safelist.fold_left
	(fun  (new_ms1,sev) (x,l) ->
	   (* under a lambda, all vars may be used recursively *)
	   let esev = clear_rec_vars (sev) in
	   let ls,new_l = sc_exp esev l in
	...
	ms
      in
	(SArrow(i, SName(i), SLens(i)), EMap(i, new_ms))
	  
  | EApp(i,e1,e2) ->
      (* applications: check that e1 has a function sort and that t2
	 has the correct param type *)
      let e1s,new_e1 = sc_exp sev e1 in
	begin
	  match e1s with
	    | SArrow(_,sa1,sa2) ->
		let e2s,new_e2 = sc_exp sev e2 in
		let _ = expect_sort sa1 e2s i in
		  sa2,EApp(i,new_e1,new_e2)
	    | _ ->
  ...
  | ELet(i,bs,e) ->
	 ...
	 sorts for bindings *)
      let bev,new_bs,_ = sc_bindings sev bs in
      let es,new_e = sc_exp bev e in
	es, ELet(i,new_bs,new_e)
  ...
  | EType(i,t) ->
      ...
      let ts,new_t = sc_typeexp sev t in
      let _ = expect_sort (SType(i)) ts in
	ts,EType(i,new_t)

  | EView(i,ks,is_list) ->
      (* views: check that dom is SNames, range SViews *)
      let new_ks = Safelist.fold_left
	(fun new_ks1 (i,n,v) -> 
 	   (* check that n has sort SName *)
	   let ns,new_n = sc_exp sev n in
	   let _ = expect_sort (SName(i)) ns in
	     (* check that v has sort SView *)
	   let vs,new_v = sc_exp sev v in
	   let _ = expect_sort (SView(i)) vs in
	     (i,new_n, new_v)::new_ks1)
	ks
	[]
      in
	SView(i),EView(i,new_ks, is_list)
	  
(** type expressions **)
(* returns its sort, and a new type expression *)
and sc_typeexp ev t = match t with
    TEmpty(i) -> SType(i),t
      
  | TName(i,e,t) ->
      let es,new_e = sc_exp ev e in
      let _ = expect_sort (SName(i)) es i in
      let ts,new_t = sc_typeexp (clear_rec_vars ev) t in 
      let _ = expect_sort (SType(i)) ts i in
	SType(i), TName(i,new_e,new_t)
	  
  | TBang(i,excl,t) ->
      let ts,new_t = sc_typeexp (clear_rec_vars ev) t in 
      let _ = expect_sort (SType(i)) ts i in
	SType(i), TBang(i,excl,new_t)
	      
  | TStar(i,excl,t) ->
      let ts, new_t = sc_typeexp (clear_rec_vars ev) t in 
      let _ = expect_sort (SType(i)) ts i in
	SType(i), TStar(i,excl,new_t)
	      
  | TCat(i,ts) ->
      let new_ts_rev =
	Safelist.fold_left
	  (fun new_ts ti ->
	     let tis, new_ti = sc_typeexp ev ti in
	     let _ = expect_sort (SType(i)) tis i in
	       (new_ti::new_ts))
	  []
	  ts	      
      in
	SType(i), TCat(i,Safelist.rev new_ts_rev)
  ...
  | TUnion(i,ts) ->
	...
	Safelist.fold_left
	  ...
	  ts
      in
	SType(i), TUnion(i,new_ts)
	  
(*       | TDiff(i,t1,t2) -> *)
(* 	  let t1s,new_t1 = sc_typeexp ev t1 in *)
(* 	  let _ = expect_sort (SType(i)) t1s i in *)
(* 	  let t2s,new_t2 = sc_typeexp ev t2 in *)
(* 	  let _ = expect_sort (SType(i)) t2s i in *)
(* 	    SType(i), TInter(i,new_t1,new_t2)       *)
(*       | TInter(i,t1,t2) -> *)
(* 	  let t1s,new_t1 = sc_typeexp ev t1 in *)
(* 	  let _ = expect_sort (SType(i)) t1s i in *)
(* 	  let t2s,new_t2 = sc_typeexp ev t2 in *)
...
and sc_bindings sev bs = 
  ...
  in
  let (new_sev, new_bs_rev, names_rev) = 
    Safelist.fold_left
      (fun (sev_rest,binds_rest,names_rest) bi ->
	 let (q,s),new_bi = sc_binding sev_rest bi in
	 let new_sev = add_rec_var q (update sev_rest q s) in
	   new_sev, new_bi::binds_rest, q::names_rest)
      (bev, [], [])
      bs
  in
    (new_sev, Safelist.rev new_bs_rev, Safelist.rev names_rev)
...
let sc_typebindings i ev tbs =
  ...
  let rec sc_typebinding ev annot_tb =
    match annot_tb with
      | xsort, (x,[],t) ->
	  let ts,new_t = sc_typeexp ev t in	    
  ...
  let new_tbs_rev = 
    ...
    Safelist.fold_left 
      (fun new_tbs annot_tbi -> (sc_typebinding tev annot_tbi)::new_tbs)
      [] 
      annot_tbs
  in
    tev, Safelist.rev new_tbs_rev, Safelist.rev names_rev
      
(* type check a single declaration *)
let rec sc_decl sev = function
  | DLet(i,bs) ->
      let bev, new_bs, names = sc_bindings sev bs in	
	clear_rec_vars bev, DLet(i,new_bs), names
...
and sc_modl_aux sev i ds : sc_env * Syntax.decl list * Syntax.qid list =
  let new_sev,new_ds_rev,names = 
    Safelist.fold_left
	 ...
	 let new_sev, new_di, new_names = sc_decl new_sev di in
	   (new_sev, new_di::ds, names @ new_names))
      (sev,[],[])
      ds
  in
