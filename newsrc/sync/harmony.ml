...
let get replica lens output =
  ...
  let cv = (Surveyor.get_reader ekey) r in
  (* now we'll get the lens *)
  let lens = Pervasives_plugin.id in
  (* apply it to get the abstract view *)
  let av = 
    match cv with
      Some cview -> Some (Lens.get lens cview)
    | None -> None in
  ...
  let writer = (Surveyor.get_writer okey) av in
  (* and do the output *)
  writer o
...
let put abstract concrete lens output =
  ...
  let okey = get_ekey o viewer_ek in
  (* apply it to get a string *)
  let writer = (Surveyor.get_writer okey) newcv in
  (* and do the output *)
  writer o
...
let sync archive replica1 replica2 schema lensa lens1 lens2 newarchive newreplica1 newreplica2 =
  ...
  let arcv = (Surveyor.get_reader arkey) ar in
  (* the 1st replica *)
  let (r1, reader_ek) = parse_replica replica1 in
  let r1key = get_ekey r1 reader_ek in
  let r1cv = (Surveyor.get_reader r1key) r1 in
  (* the 2nd replica *)
  let (r2, reader_ek) = parse_replica replica2 in
  let r2key = get_ekey r2 reader_ek in
  let r2cv = (Surveyor.get_reader r2key) r2 in
  (* the lenses *)
  let lensa = Pervasives_plugin.id in
  let lens1 = Pervasives_plugin.id in
  let lens2 = Pervasives_plugin.id in
  (* apply the lenses in the get direction *)
  let arav =
    match arcv with 
      Some v -> Some (Lens.get lensa v) | None -> None in
  let r1av =
    match r1cv with
      Some v -> Some (Lens.get lens1 v) | None -> None in
  let r2av =
    match r2cv with
      Some v -> Some (Lens.get lens2 v) | None -> None in
