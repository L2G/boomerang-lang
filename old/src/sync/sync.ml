...
type action =
  | SchemaConflict of Syntax.ctx * Syntax.typ * V.t * V.t 
  | MarkEqual
...
let rec format = function
  | SchemaConflict (delta,ty,lv,rv) -> (* need to pretty print the type too *)
      V.format_msg ([`String "[SchemaConflict] "; `String (Tutil.t2str ty);`View lv; `View rv] )
  | GoDown(m) ->
...
(* untyped and simple typed synchronizer *)
let rec sync' (delta,ty) archo lefto righto =
  let assert_member v t = 
    if (not (try (Types.member v t delta)
	     with Not_found -> 
	       begin 
		 Format.printf "ERROR while checking member for\n";
		 V.format v;
		 Format.printf "\nT:\n%s\nDELTA:\n%s\n"
		   (Tutil.t2str t) (Tutil.defs2str delta);
		 raise (Error.Type_error ("member checking failed",Error.bogusInfo))
	       end))
    then
      begin
	Format.printf "ERROR: \n view\n\t";
	...
	raise (Error.Type_error ("view is not in the schema",Error.bogusInfo))
      end;
  in    
    match (archo, lefto, righto) with
	  ...
	  if V.equal lv rv then
		...
		(fun k (actacc, aracc, lacc, racc) ->
		   let tyk = 
		     (match Types.project ty k delta with
			  None ->
	    ...
	    let acts,o',a',b' = 
	      ...
	      (V.from_list rbinds)		
	    in
	      let tdoms  = Types.tdoms ty in
	      let a'_in_tdoms = Types.vdom_in_tdoms (V.dom a') tdoms in
	      let b'_in_tdoms = Types.vdom_in_tdoms (V.dom b') tdoms in
		if a'_in_tdoms && b'_in_tdoms
		(* Old membership test: if (Types.member a' ty delta && Types.member b' ty delta) *) 
		then
		  ...
		  (SchemaConflict(delta,ty,lv,rv),archo,lefto,righto)
	   
and sync (delta,ty) o a b = 
  ...
  *)
  let result = sync' (delta,ty) o a b in
    result
