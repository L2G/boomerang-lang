...
(* We use memo information in both directions -- to short-circuit a get when we see
   it for the second time, and also to avoid computing the put when we can see what
   its result must be from the GetPut law *)
    ...
    put = (fun a co -> 
      ...
      | Some c ->
          try
            ...
            if a' == a then c else l.put a co
          with Not_found -> l.put a co)
  } 
...
let define ?(hashtable=definitions) n l =
  ...
  else Hashtbl.add hashtable n (memoize l)
