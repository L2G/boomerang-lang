...
let read_from file_name =
  try
    Some (generic_read_from (from_file file_name))
  with Sys_error s ->
    begin
      print_endline ("Warning: unable to read from " ^ file_name ^ ": " ^ s);
      None
    end

let from_string s = Some (generic_read_from (from_string s))
...
let gen_reader ?(preproc = strip_doctype) ?(tidy = false) () = fun file ->
  try
    let raw_xml = Misc.read file in
    let preproc'd_xml = preproc raw_xml in
    (*print_endline (Misc.color "Preprocessed xml:" Misc.White ~bold:true);
    print_endline preproc'd_xml;*)
    let tidy_xml =
      if tidy then
        let out,status =
          Tidy.fix_string [Tidy.ShowWarnings false;
                           Tidy.Quiet true;
                           Tidy.XmlOut true;
                           Tidy.Mark false;] preproc'd_xml in
        match status with
          Tidy.Success -> out
        | Tidy.Warning ->
            print_endline "[Xml.gen_reader] tidy issued warnings."; out
      else preproc'd_xml
    in
    from_string tidy_xml
  with Sys_error s ->
    begin
      print_endline ("Warning: unable to view file " ^ file ^ ": " ^ s);
      None
    end
...
let gen_writer ?(postproc = (fun s->s)) () vo file =
  match vo with
    None ->
      raise
      (Misc.Unimplemented "[Xml.gen_writer]: writing empty view not supported.")
  | Some v -> Misc.write file (postproc (to_string v))
