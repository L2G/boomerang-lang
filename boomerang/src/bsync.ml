...
let rec isync ty o a b = 
  let spo,tmo = Erx.parse ty o in 
  let spa,tma = Erx.parse ty a in
  let spb,tmb = Erx.parse ty b in 
  ...
  | Some(s,dir) -> begin
      ...
      let del_mod_conflict r tmr =
        Erx.TagSet.exists
          (fun ti -> Erx.TagMap.find tmr t <> Erx.TagMap.find tmo t)
          (Erx.TagSet.diff (Erx.spine_tags r) s_tags) in
      if  (dir = Left && del_mod_conflict b tmb)
       || (dir = Right && del_mod_conflict a tma)
      then (o,a,b)
      else       
        let tmo,tma,tmb =
          Erx.TagSet.fold
             ...
             let ot',at',bt' =
               Safelist.fold
                    ...
                    | Stable(oti,ati,bti) ->
                        let oti',ati',bti' =
                          let res =
                            Safelist.map
                              (fun ((k,oi),(_,ai),(_,bi)) ->
                                 let oi',ai',bi' = isync (Erx.box_type ty t) oi ai bi in
                                 (k,oi'),(k,ai'),(k,bi'))
                              (zip3 oti ati bti) in
                              unzip3 res in
                 ...
                 (TagMap.add t ot' tmo,
                  TagMap.add t at' tma,
                  TagMap.add t bt' tmb))
          (Erx.spine_tags s) (TagMap.empty,TagMap.empty,TagMap.empty) in
