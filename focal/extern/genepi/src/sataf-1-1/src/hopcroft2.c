/* $Id: hopcroft2.c,v 1.1 2006/01/04 08:15:59 point Exp $ */
...
static void s_check_elements(hopcroft_data *hd)
{
  ...
  for(i = 0; i< hd->ea->nb_local_states; i++)
    {
      e = hd->elements+i;
...
#else
exit_automaton
exit_automaton_minimize2(exit_automaton ea, uint32_t *h)
#endif
{
  uint32_t          c;
  uint32_t          a;
...
{  
  ...
  hd = ccl_calloc(sizeof(hopcroft_data)+
		  sizeof(pred)*((ea->nb_local_states)*ea->alphabet_size)+
		  sizeof(pred*)*((ea->nb_local_states)*ea->alphabet_size),1);
  hd->ea = ea;
  hd->elements = ccl_new_array(class_element,(ea->nb_local_states)); 
  hd->classes = ccl_new_array(equiv_class,ea->nb_local_states+1);
  hd->nb_classes = 0;
  hd->classes_sz = (ea->nb_local_states+1);

  hd->L = -1;
  hd->in_L = ccl_bittable_create((ea->nb_local_states+1)*ea->alphabet_size);

  hd->free_pred = hd->preds;
  hd->free_pred_table = (pred **)
    (hd->preds+((ea->nb_local_states)*ea->alphabet_size));
  
  for(e = hd->elements, s = 0; s < ea->nb_local_states; s++, e++)
    {
      e->prev = e-1;
      e->next = e+1;
      e->state = s;
  ...
  C = s_new_class(hd);
  C->size = ea->nb_local_states;

  C->elements->next = hd->elements;
  C->elements->prev = &(hd->elements[ea->nb_local_states-1]);

  hd->elements->prev = C->elements;
  hd->elements[ea->nb_local_states-1].next = C->elements;
...
s_hopcroft_terminate(hopcroft_data *hd, uint32_t *h)
{
  ...
  result = exit_automaton_create(hd->nb_classes,hd->ea->nb_exit_states,
				 hd->ea->alphabet_size);

  for(s = 0; s < hd->ea->nb_local_states; s++)
    {
      uint32_t src = h[s] = hd->elements[s].classid;

      result->is_final[hd->elements[s].classid] = hd->ea->is_final[s];
...
s_refine_wrt_block(hopcroft_data *hd, uint32_t Pindex, uint32_t a)
{
  ...
  if( ! ccl_list_is_empty(hd->tmp[1]) )
	...
	{
	  int q = (int)CAR(pc);
	  int i = hd->elements[q].classid;
	  
	  s_move_state_to(hd,q,hd->classes[i].twin);
	}
