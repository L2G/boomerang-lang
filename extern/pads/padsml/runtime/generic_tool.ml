...
let buildStringMap (lst : (string * 'a) list) : 'a StringMap.t =
  List.fold_left
    ...
    StringMap.empty
    lst
(* Wrappers for find, add functions of maps *)
let findStringMap k m = StringMap.find k m
let addStringMap k v m = StringMap.add k v m
  ...
  | RecordData (x, fields) ->
      (Xml.Element("record-data",
		   ...
		   (StringMap.fold
			...
			Xml.Element(name, [], [metadata_to_xml field_meta convert])::xml_lst)
		      fields [])))
  | DatatypeData (x, variants) ->
      (Xml.Element("datatype-data",
		   ...
		   (StringMap.fold
			...
			Xml.Element(name, [], [metadata_to_xml variant_meta convert])::xml_lst)
		      variants [])))
...
module Rec_ver = struct
  ...
  struct
	...
	datatype_t = {
	    ...
	    dt_init_empty = Tool.Datatype.Empty.init;
	    dt_process_empty = Tool.Datatype.Empty.process;
	  };
	constraint_t = {
