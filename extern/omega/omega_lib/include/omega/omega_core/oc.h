...
/* Allocation parameters and functions */
    ...
    int padEQs(int oldalloc, int newreq) {
      ...
      return std::min((newreq < 2*oldalloc ? 2*oldalloc : 2*newreq),maxmaxEQs);
    }
    int padGEQs(int oldalloc, int newreq) {
	check_number_GEQs(newreq);
	return std::min((newreq < 2*oldalloc ? 2*oldalloc : 2*newreq),maxmaxGEQs);
    }
    int padEQs(int newreq) {
	check_number_EQs(newreq);
	return std::min(std::max(newreq+first_alloc_pad,min_alloc), maxmaxEQs);
    }
    int padGEQs(int newreq) {
	check_number_GEQs(newreq);
	return std::min(std::max(newreq+first_alloc_pad,min_alloc), maxmaxGEQs);
    }
