...
Relation After(NOT_CONST Relation &input_S,
  ...
  Relation r(*S.split(),42);
    ...
    int a = r.max_ufs_arity_of_out();
    int preserved_positions = std::min(carried_by-1,new_output);
  ...
  if (carried_by > new_output) {
	  int preserve = std::min(new_output,r.n_out());
...
bool prepare_relations_for_composition(Relation &r1,
    ...
    int zeros = std::max(r1.query_guaranteed_leading_0s(),
...
Relation Project_On_Sym(NOT_CONST Relation &S,
  ...
  int d = std::min(in_arity,out_arity);
  if (d && !context.is_null()) {
        int g = std::min(d,context.query_guaranteed_leading_0s());
        int p = std::min(d,context.query_possible_leading_0s());
					 ...
					 int &in_req, int &out_req)
{
    ...
    for(i = 1;  i <= m1.n_in();  i++) {
      ...
      case Input_Var:  in_req = std::max(in_req, j);   break;
      case Set_Var:  in_req = std::max(in_req, j);   break;
      case Output_Var: out_req = std::max(out_req, j); break;
      default: break;
      }
    }
				      ...
				      int &in_req, int &out_req)
{
   ...
   in_req = std::max(inreq1, inreq2);
   out_req = std::max(outreq1, outreq2);
}
...
void Conjunct::calculate_dimensions(Relation &R, int &ndim_all, int &ndim_domain) {
   ...
   for (DNF_Iterator s(rc.query_DNF()); s.live(); s.next()) 
     n_eq_sym = std::min(n_eq_sym, s.curr()->n_EQs());
   ndim_all+=n_eq_sym;
   skip_set_checks--;
