...
void Conjunct::enforce_leading_info(int guaranteed, int possible, int dir) {
    ...
    int d = std::min(relation()->n_inp(),relation()->n_out());
...
void Conjunct::invalidate_leading_info(int changed)
	...
	leading_dir = 0;
	    ...
	    guaranteed_leading_0s = std::min(guaranteed_leading_0s,changed-1);
	}
#if ! defined NDEBUG
    assert_leading_info();
#endif
...
int Conjunct::leading_dir_valid_and_known()
    ...
    return (possible_leading_0s == guaranteed_leading_0s &&
	    ...
	    possible_leading_0s < std::min(relation()->n_inp(),relation()->n_out())
	    && leading_dir);
    }
...
void Conjunct::assert_leading_info()
	...
	&& possible_leading_0s >= 0 &&
	carried_level <= std::min(relation()->n_inp(), relation()->n_out()))
	{
	Variable_ID in = input_var(carried_level),
	    out = output_var(carried_level);
	coef_t lb, ub;
...
Conjunct* merge_conjs(Conjunct* conj1, Conjunct* conj2,
    ...
    if (action == MERGE_COMPOSE)
	{
	conj3->guaranteed_leading_0s=std::min(conj1->guaranteed_leading_0s,
					 conj2->guaranteed_leading_0s);
	conj3->possible_leading_0s=std::min((unsigned int) conj1->possible_leading_0s,
	...
	if (conj3->leading_dir == 0)
	    conj3->possible_leading_0s = std::min(conj3->relation()->n_inp(),
	...
	int feasable = 1;
        ...
        int redAndBlackGuarLeadingZeros = std::max(conj1->guaranteed_leading_0s,
					 conj2->guaranteed_leading_0s);
	...
	conj3->possible_leading_0s=std::min((unsigned)conj1->possible_leading_0s,
