...
// copy from their original column (using find_column) to the new one.
    ...
    if(!reverseOrder) {
	    for(i=1; i<=std::min(myRelation->n_inp(),first_pass_input);i++)
		if (input_used[i]) newpos.append(input_vars[i]);
	    for(i=1; i<=std::min(myRelation->n_out(),first_pass_output);i++)
		if (output_used[i]) newpos.append(output_vars[i]);
	    for(i=std::max(1,first_pass_input+1); i<=myRelation->n_inp();i++)
		if (input_used[i]) newpos.append(input_vars[i]);
	    for(i=std::max(1,first_pass_output+1); i<=myRelation->n_out();i++)
		if (output_used[i]) newpos.append(output_vars[i]);
	    }
    else {
	    for(i=1; i<=std::min(myRelation->n_out(),first_pass_output);i++)
