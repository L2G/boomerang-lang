...
#define foreach_map(k,K,v,V,M,A) {for (MapElementIterator<K,V> __M_##k = (M).iterator();__M_##k;__M_##k++) {K & k = *__M_##k; V & v = __M_##k.oc_value(); A;}}
...
public:
    ...
    virtual const V &  oc_value() const       { return i->v; }
    virtual	  V &  oc_value()             { return i->v; }
